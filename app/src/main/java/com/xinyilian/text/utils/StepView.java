...
/**
 * Created by 马天 on 2018/10/17.
 * description：
 */
public class StepView extends View {
    private static final int minHorizontalSpace = 40;  //左右两侧保留的最小距离
    private int circleRadius = 8;  //圆半径
    private float circleMultiple = 1.2f;    //当前进度圆与正常圆的比例
    private int mWidth;
    ...
    private Paint circleStrokePaint;  //画环画笔
    private Paint textPaint;    //文字画笔
    private Paint linePaint;    //进度线画笔
    private int stepNum = 0;
    private int currentStep = 0;    //当前步骤(从0开始)
    private int lineDoneColor = Color.parseColor("#E83A0F");
    private int lineDefaultColor = Color.parseColor("#CCCCCC");
    private int strokeCircleSelectColor = Color.parseColor("#ED9E8E");
    private int circleSelectColor = Color.parseColor("#E83A0F");
    private int circleDefaultColor = Color.parseColor("#CCCCCC");
    private int textSelectColor = Color.parseColor("#666666");
    private int textDefaultColor = Color.parseColor("#CCCCCC");
    ...
    private void init() {
        ...
        circleStrokePaint.setColor(strokeCircleSelectColor);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int mHeight = (int) Math.min(heightSize, 2 * circleMultiple * circleRadius + 80) + paddingTop + paddingBottom;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < stepNum; i++) {
            ...
            RectF rectf = new RectF(lineLeft, mY - 1, lineRight, mY + 1);
        ...
        for (int i = 0; i < stepNum; i++) {
            ...
            canvas.drawCircle(x, mY, i == currentStep ? circleRadius * circleMultiple : circleRadius, circlePaint);
            if (i == currentStep) {
                circleStrokePaint.setColor(i <= currentStep ? strokeCircleSelectColor : circleDefaultColor);
                x = (float) ((i * 2 + 1) * circleRadius + i * space) + paddingLeft;
                canvas.drawCircle(x, mY, i == currentStep ? (circleRadius) * circleMultiple : circleRadius, circleStrokePaint);
            }
